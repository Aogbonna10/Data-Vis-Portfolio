...
def get_last_edit(soup):
    ...
    * Finally, we don't want the whole text, 
    ...
    found_element = soup.find(id="footer-info-lastmod")
    print(found_element)
    print(soup.text[0:100])
    last_edit = soup.find(id="footer-info-lastmod").text
    start_cut = last_edit.find(" on ")
    end_cut = last_edit.find(", at")
    last_edit_fixed = last_edit[start_cut+4:end_cut]
    print(last_edit_fixed)
    return last_edit_fixed 
...
def get_wordcount(soup):
    ...
    all_text = soup.text
    inner_text = all_text.split()
    return len(inner_text)
...
def get_all_wiki_link_ids(soup):
    ...
    * don't have ":" or "#"
    ...
    for link in all_links:
        if ':' not in link and '#' not in link and link.startswith(prefix):
            spliced_link = link[6:]
            wiki_links.append(spliced_link)
...
def get_shortest_page(page_directory):
    ...
    for page in page_directory:
        value = page_directory[page]
        word_count = value.wordcount
        if word_count < winning_count:
            winner = value
            winning_count = word_count
...
def get_oldest_page(page_directory):
    ...
    for page in page_directory:
        ...
        timestamp = datetime.strptime(date, '%d %B %Y' )
...
class WikiPage:
    def __init__(self, page_directory, page_id):
        ...
        self.page_directory[page_id] = self 
    ...
    def load_links(self, recursion_count=0, links_per_article=5):
        ...
        short_list = self.wiki_links[0:links_per_article]
        for wiki_id in short_list:
            if wiki_id not in self.page_directory:
                instance = WikiPage(self.page_directory, wiki_id)
                if recursion_count > 0 :
                    recursion_count = recursion_count - 1
                    instance.load_links(recursion_count, 
...
if __name__ == "__main__":
    ...
    expected_cat_count = 15380
    expected_cat_edit = "13 November 2021"
